...
)
{
	...
	{
		case SD_CARD://SD卡
		 	if( SPI_SD_Init() != SPI_SD_OK )//STM32 SPI的bug,在sd卡操作失败的时候如果不执行下面的语句,可能导致SPI读写异常
			{
...
)
{
	...
	{
		case SD_CARD://SD卡
			res = SPI_SD_ReadDisk( buff,sector,count );
...
)
{
	...
	{
		case SD_CARD://SD卡
			res = SPI_SD_WriteDisk((u8*)buff,sector,count);
			break;
		case EX_FLASH://外部flash
//			for(;count>0;count--)
//			{										    
	...
	if(pdrv==SD_CARD)//SD卡
	{
	    switch(cmd)
	    {
		    ...
		    case GET_SECTOR_COUNT:
                if( SPI_SD_GetSectorCount()==SPI_SD_OK ){
                    *(DWORD*)buff = SPI_SD.capacity;
                    res = RES_OK;
                }else
                {
                    res = RES_ERROR;
